...
export default function CrearPeluqueriaForm({ theme, onSuccess }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      {mensaje && <p className={`mb-4 text-center font-bold ${theme === "dark" ? "text-green-600" : "text-green-400"}`}>{mensaje}</p>}
      <input name="nombre" value={form.nombre} onChange={handleChange} placeholder="Nombre" className={`w-full border p-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-400 ${theme === "dark" ? "bg-gray-100 text-gray-900 placeholder-gray-500" : "bg-gray-800 text-white placeholder-gray-400"}`} required />
      <input name="direccion" value={form.direccion} onChange={handleChange} placeholder="Dirección" className={`w-full border p-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-400 ${theme === "dark" ? "bg-gray-100 text-gray-900 placeholder-gray-500" : "bg-gray-800 text-white placeholder-gray-400"}`} required />
      <input name="telefono" value={form.telefono} onChange={handleChange} placeholder="Teléfono" className={`w-full border p-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-400 ${theme === "dark" ? "bg-gray-100 text-gray-900 placeholder-gray-500" : "bg-gray-800 text-white placeholder-gray-400"}`} required />
      <textarea name="descripcion" value={form.descripcion} onChange={handleChange} placeholder="Descripción" className={`w-full border p-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-400 ${theme === "dark" ? "bg-gray-100 text-gray-900 placeholder-gray-500" : "bg-gray-800 text-white placeholder-gray-400"}`} required />
      {/* Imagen principal */}
      <div>
        ...
        {form.peluqueros.map((peluquero, i) => (
          <div key={i} className="flex gap-2 mb-2">
            <input
              ...
              className={`w-full border p-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-400 ${theme === "dark" ? "bg-gray-100 text-gray-900 placeholder-gray-500" : "bg-gray-800 text-white placeholder-gray-400"}`}
            />
      ...
      <div>
        ...
        {form.productos.map((producto, i) => (
          <div key={i} className={`mb-2 border p-2 rounded ${theme === "dark" ? "bg-gray-50" : "bg-gray-900/80"}`}>
            <input
              value={producto.nombre}
              onChange={e => handleArrayChange(e, i, "productos", "nombre")}
              placeholder="Nombre del producto"
              className={`w-full border p-2 rounded mb-1 focus:outline-none focus:ring-2 focus:ring-blue-400 ${theme === "dark" ? "bg-gray-100 text-gray-900 placeholder-gray-500" : "bg-gray-800 text-white placeholder-gray-400"}`}
            />
            <input
              type="file"
              accept="image/*"
